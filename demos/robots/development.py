...
def send_to_simulator(sim, weight_matrix, devo_matrix):
    ...
    for i in range(4):
        ...
        #
        # # slide1 to slide2
        # knees[i] = sim.send_hinge_joint(slide_cyls[i], slide_cyls[i+4],
        ...
        #
        # # shin part that touches floor
        # shins[i] = sim.send_cylinder(x=x_pos2, y=y_pos2, z=(HEIGHT + EPS) / 2.0,
        #                              r1=0, r2=0, r3=1,
        #                              length=HEIGHT, radius=EPS
        #                              )
        # # slide2 to shin
        # slide_joints[i+4] = sim.send_slider_joint(slide_cyls[i+4], shins[i], x=0, y=0, z=HEIGHT+EPS)
        #
        # # attach slide motor later
        #
    ...
    env_box = sim.send_box(x=5, y=-5, z=HEIGHT/2.0,
                           length=HEIGHT,
                           width=HEIGHT,
                           height=HEIGHT,
                           mass=3.)
...
if __name__ == "__main__":
    ...
    dist = sim.get_sensor_data(layout['light_sensor'])
    print dist[-1]
